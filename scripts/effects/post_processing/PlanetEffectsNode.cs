...
public partial class PlanetEffectsNode : Node
{
    ...
    [Export] public bool DisplayAtmosphere { get; set; } = false;
    ...
    public override void _Ready()
    {
        ...
        _atmosphereTargetMesh = AtmosphereTargetMeshPath == default
            ? default
            : GetNode<MeshInstance3D>(AtmosphereTargetMeshPath);
        _atmosphereViewport = AtmosphereViewportPath == default ? default : GetNode<Viewport>(AtmosphereViewportPath);

        _oceanTargetMesh = OceanTargetMeshPath == default
            ? default
            : GetNode<MeshInstance3D>(OceanTargetMeshPath);
        _oceanViewport = OceanViewportPath == default ? default : GetNode<Viewport>(OceanViewportPath);

        _sourceViewport = SourceViewportPath == default ? default : GetNode<Viewport>(SourceViewportPath);
    }
    ...
    public override void _Process(double delta)
    {
        ...
        if (_generator != default && _effectsHolder == default)
        {
        ...
        if (_sourceViewport is not { } sourceViewport)
        {
            return;
        }
        ...
        {
            atmosphereEffect.UpdateSettings(sourceViewport, _effectsHolder.Generator, atmosphereShader);
        }
        ...
        {
            oceanEffect.UpdateSettings(sourceViewport, _effectsHolder.Generator, oceanShader);
        }
    }
}