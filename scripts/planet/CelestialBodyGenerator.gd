...
@export var body : Resource :
	...
	set(val):
		...
		body.ShadingChanged.connect(on_shade_data_changed)
		body.ShapeChanged.connect(on_shape_data_changed)
...
func _process(_delta):
	if Engine.is_editor_hint():
		...
		if lod_meshes.size() > 0:
			for i in range(resolution_settings.LodParameters.size()):
				var lod : LODParameter = resolution_settings.LodParameters[i]
...
func handle_game_mode_generation():
	if can_generate_mesh():
		...
		if body.Shading != null:
			...
			body.Shading.SetTerrainProperties(
...
func handle_edit_mode_generation():
	if can_generate_mesh():
		...
		if shading_noise_settings_updated and body.Shading != null:
			...
			$MeshInstance3d.material_override = body.Shading.TerrainMaterial.duplicate()
			#body.Shading.initialize(body.Shape)
			body.Shading.SetTerrainProperties(
				$MeshInstance3d.material_override, height_min_max, body_scale())
			...
			if not shape_settings_updated:
				...
				# Shading noise data
				body.Shading.initialize(body.Shape)
				var shading_data = body.Shading.generate_shading_data(arrays[Mesh.ARRAY_VERTEX])
...
func can_generate_mesh() -> bool:
	return body != null and body.Shape != null and body.Shape.HeightMapCompute != null
...
func generate_terrain_mesh(_mesh : ArrayMesh, resolution : int) -> Vector2:
	...
	# Set heights
	var heights : PackedFloat32Array = body.Shape.CalculateHeights(vertices)

	# Perturb vertices to give terrain a less perfectly smooth appearance
	if body.Shape.PerturbVertices and body.Shape.PerturbCompute != null:
		var perturb : Resource = body.Shape.PerturbCompute
		var max_perturb_strength : float = body.Shape.PerturbStrength * edge_length / 2.0
...
func pick_terrain_res() -> int:
	if Engine.is_editor_hint():
		if preview_mode == PreviewMode.LOD0 and resolution_settings.LodParameters.size() > 0:
			return resolution_settings.LodParameters[0].Resolution
		if preview_mode == PreviewMode.LOD1 and resolution_settings.LodParameters.size() > 1:
			return resolution_settings.LodParameters[1].Resolution
		if preview_mode == PreviewMode.LOD2 and resolution_settings.LodParameters.size() > 2:
			return resolution_settings.LodParameters[2].Resolution
		if preview_mode == PreviewMode.CollisionRes:
			return resolution_settings.Collider
	return 0

func get_ocean_radius() -> float:
	if (body.Shading.OceanSettings != null and
		body.Shading.OceanSettings.Enabled
	):
		return unscaled_ocean_radius() * body_scale()
	return 0.0

func unscaled_ocean_radius() -> float:
	return lerp(height_min_max.x, 1.0, body.Shading.OceanSettings.Level)
...
func point_on_planet(dir_from_origin : Vector3):
	...
	var height : PackedFloat32Array = body.Shape.CalculateHeights(vertex)
